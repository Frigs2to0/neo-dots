...
function createRoom(config) {
  const { bans = 2, picks = 6, timerSeconds = 40, reserveSeconds = 60, ambarName = "Âmbar", safiraName = "Safira" } = config || {}
  ...
  const room = {
    id,
    config: { bans, picks, timerSeconds, reserveSeconds, ambarName, safiraName },
    tokens,
    clients: new Set(),
    state: {
      started: false,
...
function getPublicState(room) {
  ...
  return {
    ...
    ambarName: room.state.ambarName,
    safiraName: room.state.safiraName,
  }
}

function setTeamName(room, team, name) {
  if (room.state.started) return { error: "Draft já iniciado" }
  if (team !== "ambar" && team !== "safira") return { error: "Time inválido" }
  if (!name || name.trim() === "") return { error: "Nome inválido" }

  const key = team === "ambar" ? "ambarName" : "safiraName"
  room.state[key] = name.trim()
  room.config[key] = name.trim()
